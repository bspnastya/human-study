...
def make_questions() -> List[Dict]:
    ...
    ordered, prev = [], None
    while pool:
        options = [q for q in pool if q["group"] != prev] or pool
...
def finish(ans: str):
    ...
    st.session_state.blank_until = time.time() + 1
    st.experimental_rerun()
...
Сейчас вы увидите изображение. Цель данного вопроса — посмотреть на диаметрально
противоположные углы, <b>правый верхний и левый нижний</b>, и определить,
