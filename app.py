...
components.html(mobile_check_script, height=0)
...
log_q = queue.Queue(maxsize=100)

def writer():
    while True:
        try:
            r = log_q.get(timeout=QUEUE_TIMEOUT)
            if r is None:
                break
            if SHEET:
                retry_count = 0
                while retry_count < MAX_RETRIES:
                    try:
                        SHEET.append_row(r)
                        logger.info(f"Successfully logged data for user: {r[1]}")
                        break
                    except Exception as e:
                        retry_count += 1
            ...
            logger.error(f"Critical error in writer thread: {e}")
...
if (st.session_state.pause_until > current_time and st.session_state.idx < len(st.session_state.questions)):
    ...
    if current_time - st.session_state.get('last_refresh', 0) > 0.4:
...
def finish(a: str):
    try:
        ...
        logger.error(f"Error in finish function: {e}")
        st.error("Произошла ошибка. Пожалуйста, обновите страницу.")
...
with st.container():
    if remaining > 0:
        ...
        components.html(img_html, height=310)
    else:
