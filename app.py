...
def make_qs():
    ...
    seq,prev=[],None
    while any(pg.values()):
        available=[g for g in GROUPS if pg[g] and g!=prev] or [g for g in GROUPS if pg[g]]
        prev=random.choice(available); seq.append(pg[prev].pop())
...
def log_row(ans):
    ...
    row=[datetime.datetime.utcnow().isoformat(), st.session_state.name, q["№"], q["group"],
         q["alg"], q["qtype"], q["prompt"], ans, q["correct"], ms, ok, st.session_state.session_id]
