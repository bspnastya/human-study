...
if "_timer_slots" not in st.session_state:
    st.session_state["_timer_slots"] = {}
...
def render_timer(sec: int, tid: str):
    slots = st.session_state["_timer_slots"]
    if tid not in slots:
        slots[tid] = st.empty()
        slots[tid].components.html(
            f"""
...
if "_local_q" not in st.session_state:
    st.session_state["_local_q"] = queue.Queue()
    def writer(local_q):
        while True:
            row = local_q.get()
            if SHEET:
...
render_timer(math.ceil(remaining), f"q{idx}")
