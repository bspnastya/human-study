...
<script>
(function(){{
 const dash=163.36,ttl={sec};let left=ttl;
 const bar=document.getElementById("bar-{key}"),lbl=document.getElementById("lbl-{key}"),txt=document.getElementById("txt-{key}");
 function tick(){{left--;if(left<0)return;lbl.textContent=left;bar.style.strokeDashoffset=dash*left/ttl;if(txt)txt.textContent="{prefix}"+left+" с";setTimeout(tick,1000);}}
...
def make_questions()->List[Dict]:
    ...
    for g,a in itertools.product(GROUPS,ALGS):
        per_group[g]+=[dict(group=g,alg=a,img=file_url(g,a),qtype="corners",prompt="Правый верхний и левый нижний угол — одного цвета?",correct=CORNER_ANS[g]),
...
if "questions" not in st.session_state:
    st.session_state.update(questions=make_questions(),idx=0,name="",phase="intro",
                            hard_timer=None,start_time=None)
...
def finish(ans:str):
    q=qs[st.session_state.idx];ms=int((time.time()-st.session_state.start_time)*1000) if st.session_state.start_time else 0
    ...
    st.session_state.phase="intro";st.session_state.hard_timer=None;st.session_state.start_time=None
...
if i<total_q:
    ...
    if st.session_state.phase=="intro":
        dur=8 if i<5 else 2
        if st.session_state.hard_timer is None:
            st.session_state.hard_timer=time.time()+dur
            st_autorefresh(interval=dur*1000+100,limit=1,key=f"intro-jump-{i}")
        left=st.session_state.hard_timer-time.time()
        html_timer(max(math.ceil(left),0),key=f"intro{i}",prefix="Начало показа через ")
        if left<=0:
            st.session_state.phase="question";st.session_state.hard_timer=None;st.experimental_rerun()
        if q["qtype"]=="corners":
            st.markdown("""<div style="font-size:1.1rem;">… (ИНСТРУКЦИЯ УГЛЫ) …</div>""",unsafe_allow_html=True)
        else:
            st.markdown("""<div style="font-size:1.1rem;">… (ИНСТРУКЦИЯ БУКВЫ) …</div>""",unsafe_allow_html=True)
        st.stop()

    if st.session_state.hard_timer is None:
        st.session_state.hard_timer=time.time()+TIME_LIMIT
        st.session_state.start_time=time.time()
        st_autorefresh(interval=TIME_LIMIT*1000+100,limit=1,key=f"q-jump-{i}")
    left=st.session_state.hard_timer-time.time()
    html_timer(max(math.ceil(left),0),key=f"q{i}")
