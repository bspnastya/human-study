...
if "initialized" not in st.session_state:
    st.session_state.update(initialized=True, questions=[], idx=0, name="",
                            ...
                            timer_shown=False, session_id=secrets.token_hex(8))
...
def backup_row(r):
    BACKUP_DIR.joinpath(f"{int(time.time()*1e6)}.json").write_text(
...
if not globals().get("_WRITER_STARTED"):
    ...
    def writer():
        ...
        while True:
            try:
                row = global_queue.get(timeout=1)
                buf.append(row)
            except queue.Empty:
                pass
...
def timer(sec):
    ...
    <div style="font-size:1.2rem;font-weight:bold;color:#111;margin-bottom:10px;">
      Осталось&nbsp;времени: <span id="tt">{sec}</span>&nbsp;сек
    </div>
    <script>
      let t={sec};const s=document.getElementById('tt');
...
def log_next(ans):
    ...
    global_queue.put([datetime.datetime.utcnow().isoformat(), st.session_state.name,
                      q["№"], q["group"], q["alg"], q["qtype"], q["prompt"], ans,
                      q["correct"], ms, ok, st.session_state.session_id])
...
if st.session_state.pause_until > time.time():
    components.html("<div style='text-align:center;font-size:1.5rem;color:#fff;background:#262626;padding:20px;border-radius:12px;margin-top:50px;'>Переходим к следующему вопросу…</div><script>setTimeout(()=>parent.location.reload(),600);</script>", height=120)
    st.stop()
...
if q["qtype"] == "corners":
    choice = st.radio(q["prompt"], ["Да, углы одного цвета.", "Нет, углы окрашены в разные цвета.", "Затрудняюсь ответить."], index=None, key=f"ch_{idx}")
    if choice:
        log_next("да" if choice.startswith("Да") else "нет" if choice.startswith("Нет") else "затрудняюсь")
else:
    ans = st.text_input(q["prompt"], key=f"ans_{idx}", placeholder="Введите буквы и Enter")
    if st.button("Не вижу букв", key=f"none_{idx}"):
        log_next("Не вижу")
    if ans and re.fullmatch(r"[А-Яа-яЁё ,.;:-]+", ans):
        log_next(ans.strip())
    elif ans:
