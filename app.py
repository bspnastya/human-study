...
def timer_block(sec:int,key:str="",label:str=""):
    return f"""
<div style='display:flex;gap:16px;align-items:center;height:70px'>
  <div style='position:relative;width:70px;height:70px'>
    <svg width='70' height='70'><circle cx='35' cy='35' r='26' stroke='#444' stroke-width='6' fill='none'/><circle id='bar-{key}' cx='35' cy='35' r='26' stroke='#52b788' stroke-width='6' fill='none' stroke-dasharray='163.36' stroke-dashoffset='0' transform='rotate(-90 35 35)'/></svg>
    <span id='lbl-{key}' style='position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font:700 1.2rem sans-serif;color:#52b788'>{sec}</span>
  </div>
  {f"<div id='txt-{key}' style='font:500 1rem sans-serif;color:#52b788;'>{label}{sec}&nbsp;с</div>" if label else ""}
</div>
<script>
(function(){{
 let left={sec};
 ...
 function t(){{left--;if(left<0)return;lbl.textContent=left;bar.style.strokeDashoffset=dash*(1-left/{sec});if(txt)txt.textContent="{label}"+left+" с";setTimeout(t,1000);}}
 setTimeout(t,1000);
}})();
</script>"""
...
def make_questions()->List[Dict]:
    ...
    while any(per.values()):
        ...
        g=random.choice(ch); out.append(per[g].pop()); prev=g
    for n,q in enumerate(out,1): q["№"]=n
    return out

if "questions" not in st.session_state:
    st.session_state.update(questions=make_questions(),idx=0,name="",phase="intro",deadline=0,q_deadline=0,q_start=None)

qs,total_q=st.session_state.questions,len(st.session_state.questions)

if not st.session_state.name:
    st.markdown("""<div style="color:#111;">(ваш полный текст стартовой страницы)</div>""",unsafe_allow_html=True)
...
def finish(ans:str):
    ...
    st.session_state.idx+=1; st.session_state.phase="intro"; st.session_state.deadline=0; st.session_state.q_deadline=0; st.rerun()
...
if i<total_q:
    ...
    if st.session_state.phase=="intro":
        ...
        if st.session_state.deadline==0: st.session_state.deadline=time.time()+dur
        remain=max(int(st.session_state.deadline-time.time()),0)
        components.html(timer_block(remain,key=f"intro{i}",label="Начало показа через "),height=80)
        if remain==0: st.session_state.phase="question"; st.rerun()
        if q["qtype"]=="corners":
    ...
    st.markdown("""<div style="margin-top:30px;padding:30px;text-align:center;font-size:2rem;color:#fff;background:#262626;border-radius:12px;">Вы завершили прохождение.<br><b>Спасибо за участие!</b></div>""",unsafe_allow_html=True); st.balloons()
