...
def make_questions()->List[Dict]:
    ...
    for v in bag.values(): random.shuffle(v)
    out=[]
    while any(bag.values()):
        for g in random.sample(GROUPS,len(GROUPS)):
            if bag[g]: out.append(bag[g].pop())
...
if i<total_q:
    ...
    components.html(f"""<div style="display:flex;gap:16px;height:70px">
      <div style="position:relative;width:70px;height:70px">
        ...
        stroke-dasharray="163.36" stroke-dashoffset="{163.36*(left/TIME_LIMIT)}" transform="rotate(-90 35 35)"/></svg>
        <span style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);
        font:700 1.2rem sans-serif;color:#52b788">{left}</span></div></div>""",height=80)

    st.markdown(f"### Вопрос №{q['№']} из {tot}")
