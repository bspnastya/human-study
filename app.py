...
def ring(sec:int,key:str="",prefix:str=""):
    components.html(f"""
...
if "questions" not in st.session_state:
    st.session_state.update(questions=make_questions(),idx=0,name="",phase="intro",
                            deadline=None,start=None)

qs,total_q=st.session_state.questions,len(st.session_state.questions)

if not st.session_state.name:
    st.markdown("""<div style="color:#111;">… (полный неизменённый текст вступления) …</div>""",unsafe_allow_html=True)
...
def finish(ans:str):
    q=qs[st.session_state.idx];ms=int((time.time()-st.session_state.start)*1000) if st.session_state.start else 0
    ...
    st.session_state.phase="intro";st.session_state.deadline=None;st.session_state.start=None;st.rerun()
...
if i<total_q:
    ...
    if st.session_state.phase=="intro":
        ...
        if st.session_state.deadline is None:
            ...
            st_autorefresh(interval=int(dur*1000)+100,limit=1,key=f"intro-jump-{i}")
        left=st.session_state.deadline-time.time()
        ring(max(math.ceil(left),0),key=f"intro{i}",prefix="Начало показа через ")
        if left<=0:
            st.session_state.phase="question";st.session_state.deadline=None;st.rerun()
    ...
    left=st.session_state.deadline-time.time()
    ring(max(math.ceil(left),0),key=f"q{i}")
