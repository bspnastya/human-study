...
import random, time, datetime, secrets, threading, queue, re, itertools, math, requests
...
def make_questions() -> List[Dict]:
    ...
    for g,a in itertools.product(GROUPS,ALGS):
        per[g].append(dict(group=g,alg=a,img=file_url(g,a),qtype="corners",
                           prompt="Правый верхний и левый нижний угол — одного цвета?",correct=CORNER_ANS[g]))
        per[g].append(dict(group=g,alg=a,img=file_url(g,a),qtype="letters",
    ...
    while any(per.values()):
        groups=[g for g in GROUPS if per[g] and g!=prev] or [g for g in GROUPS if per[g]]
        g=random.choice(groups); out.append(per[g].pop()); prev=g
    for n,q in enumerate(out,1): q["№"]=n
    return out

if "questions" not in st.session_state:
    st.session_state.update(questions=make_questions(), idx=0, name="",
                            phase="intro", deadline=None, start=None)
...
""", unsafe_allow_html=True)
    u = st.text_input("", placeholder="Фамилия / псевдоним",
    ...
    if st.button("🎲 Сгенерировать псевдоним"):
        ...
        st.rerun()
    if u:
        st.session_state.name = u.strip(); st.rerun()
    st.stop()
...
def finish(ans:str):
    ...
    if SHEET: log_q.put([datetime.datetime.utcnow().isoformat(),st.session_state.name,
                         q["№"],q["group"],q["alg"],q["qtype"],q["prompt"],ans,
                         q["correct"],ms,ok])
...
if i < total_q:
    ...
    if st.session_state.phase == "intro":
        ...
        if st.session_state.deadline is None:
            ...
            st_autorefresh(interval=dur*1000+100, limit=1, key=f"introJump{i}")
...
На некоторых картинках букв нет — тогда нажмите кнопку <b>«Не&nbsp;вижу&nbsp;букв»</b>.
    ...
    if st.session_state.deadline is None:
        ...
        st_autorefresh(interval=TIME_LIMIT*1000+100, limit=1, key=f"qJump{i}")
    left = st.session_state.deadline - time.time()
    ...
    if q["qtype"] == "corners":
        sel = st.radio(q["prompt"],("Да, углы одного цвета.","Нет, углы окрашены в разные цвета."),
