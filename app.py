...
import random, time, datetime, secrets, threading, queue, re, itertools, requests, math
...
<div style="display:flex;gap:16px;height:70px">
  ...
  {"<div style='font:500 1rem sans-serif;color:#52b788;align-self:center;'>"+prefix+str(left)+" с</div>" if prefix else ""}
</div>
""",
        height=80,
    )
...
def make_questions() -> List[Dict]:
    ...
    for g,a in itertools.product(GROUPS,ALGS):
        per[g].append(dict(group=g,alg=a,img=file_url(g,a),
                           qtype="corners",prompt="Правый верхний и левый нижний угол — одного цвета?",correct=CORNER_ANS[g]))
        per[g].append(dict(group=g,alg=a,img=file_url(g,a),
                           qtype="letters",prompt="Если на изображении вы видите буквы, то укажите, какие именно.",correct=LETTER_ANS[g]))
    ...
    while any(per.values()):
        ...
        g=random.choice(choose); order.append(per[g].pop()); prev=g
    for n,q in enumerate(order,1): q["№"]=n
    return order

if "questions" not in st.session_state:
    st.session_state.update(questions=make_questions(), idx=0, name="",
                            phase="intro", start_time=None, intro_total=None, q_start=None)
...
def finish(ans:str):
    ...
    st.session_state.intro_total = None
...
if i < total_q:
    ...
    if st.session_state.phase == "intro":
        ...
        if st.session_state.start_time is None:
            ...
            st.session_state.intro_total = limit
        left_intro = max(st.session_state.intro_total - (time.time()-st.session_state.start_time), 0)
        ring(int(math.ceil(left_intro)), st.session_state.intro_total, key=f"intro{i}", prefix="Начало показа через ")
...
На некоторых картинках букв нет — тогда нажмите кнопку <b>«Не&nbsp;вижу&nbsp;букв»</b>.
    ...
    left_q = max(TIME_LIMIT - (time.time()-st.session_state.q_start), 0)
    ring(int(math.ceil(left_q)), TIME_LIMIT, key=f"q{i}")
