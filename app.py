...
<div style="display:flex;gap:16px;height:70px">
  ...
  {"<div id='txt-"+key+"' style='font:500 1rem sans-serif;color:#52b788;align-self:center;'>"+prefix+str(total_sec)+" с</div>" if prefix else ""}
</div>
<script>
(function(){{
...
if i < total_q:
    ...
    if st.session_state.phase == "intro":
        ...
        raw_left = intro_limit - (time.time() - st.session_state.intro_start)
        secs_display = max(math.ceil(raw_left), 0)
        html_timer(secs_display, key=f"intro{i}", prefix="Начало показа через ")
        if raw_left <= 0:
...
На некоторых картинках букв нет — тогда нажмите кнопку <b>«Не&nbsp;вижу&nbsp;букв»</b>.
    ...
    secs_display = max(math.ceil(raw_left), 0)
    html_timer(secs_display, key=f"q{i}")
