...
.main,.block-container{background:#808080!important;color:#111!important;}
...
def finish(ans: str):
    ...
    ms = (
        int((time.time() - st.session_state.start_time) * 1000)
...
if st.session_state.phase == "intro":
    ...
    if remain > 0:
        ...
        render_timer_js(remain, f"intro{i}", send_done=True)
...
if left > 0:
    ...
    st.markdown(
        "<i>Время показа изображения истекло.</i>", unsafe_allow_html=True
    )
...
if q["qtype"] == "corners":
    ...
    if sel:
        finish(
            ...
            if sel.startswith("Нет")
            else "затрудняюсь"
        )
else:
    txt = st.text_input(
        q["prompt"], key=f"in{i}", placeholder="Введите русские буквы"
    )
